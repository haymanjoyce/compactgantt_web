...
**The colophon has no Excel presence at all** — not its dimensions, not its colors, not its existence. The Titles sheet is the six header/footer rows and nothing else. See Renderer.
...
| `'unrecognised_boolean'` | non-empty string that trimmed+lowercased is neither `'yes'` nor `'no'`. Native booleans pass silently. |
| `'unrecognised_enum'` | non-empty value a `normalize*` function didn't recognise. |
| `'id_assigned'` | id cell blank/missing/unparseable; parser assigned `max(existing)+1`. Suppresses the upstream `unparseable_number`, and fires on empty id cells too — the exception to "empty produces no notice". |
...
- **Vertical band stack:** bottom-up `paddingBottom` → colophon → footer → task rows, derived as ONE chain (`colophonY`, then `footerY = colophonY - footerHeight`, then `taskRowY2 = footerY`). `footerY` is simultaneously the footer band's top edge and the task-row bottom, so **the footer band consumes `footerY` and must never recompute its own `y`** — they were duplicate expressions before the colophon existed, and re-deriving either silently overlaps the band. `taskRowY2` is the single source for ~9 downstream sites (gridlines, pipes, curtains, badge areas, `taskRowH` → `rowH` → notes and baseline), so band heights propagate from that one line. `validateLayout` mirrors the sum — see Validation.
...
- **Swimlane backgrounds/labels:** `<rect>` fill = `swimlane.backgroundColor` (parser default `"white"`, no renderer fallback). Label styling via three `config.typography` booleans — `swimlaneLabelBold` (default `true`), `swimlaneLabelItalic`/`swimlaneLabelUnderline` (default `false`): `font-weight` always emitted, `font-style`/`text-decoration` only when true, so all-default output stays byte-identical to the former hard-coded bold.
...
- **Colophon (slot 16, group `colophon`, last):** structural brand band across the bottom of the perimeter, below the footer (or below the task rows when `footerHeight` is 0), spanning the header/footer extent. **Unconditional, and with no exposed field of any kind** — not presence, not size, not color. It reserves its own space in the band stack, so nothing can overlap it and no config can remove it; the only escape is cropping the image. Right-aligned fixed literal `compactgantt.com`, pure ASCII. Top-edge border mirrors the header/footer band rule. **Colors are the fixed `rendering.colophon*Color` constants and deliberately do NOT track `config.style`** — an earlier design inherited the footer's palette, which made the band illegible against a restyled footer; the band must stay readable whatever the user does elsewhere. The early-return guards emit no colophon — the one deliberate exception to "always present".
...
- **Bars/pattern fills:** `task.fillPattern` → dedup'd `<pattern>` defs keyed by `(fillPattern, fillColor, patternColor)`; `"solid"`/unrecognised → solid. `patternUnits="userSpaceOnUse"` with no `x`/`y`, so same-row bars share a continuous-field phase. Milestones always solid.
...
The colophon's six constants live here, and are its **only** configuration (`colophonHeight` `12`, `colophonFontSize` `6`, `colophonAlignmentFactor` `0.7`, `colophonBackgroundColor` `#f5f5f5`, `colophonBorderColor` `#d0d0d0`, `colophonTextColor` `#5a5a5a`). `12`/`6` holds header/footer's 20-to-10 band-to-font ratio; baseline `12 × 0.7 = 8.4` leaves ~4.2px above the caps and ~2.4px below the descenders of `compactgantt.com`. **6px is the floor** — 5px collapses the cap height to 3.5px and antialiasing destroys the URL. The text grey is ~6.3:1 on the band background: since nothing can restyle these, the contrast has to hold unconditionally, so don't lighten it. **`colophonAlignmentFactor` is deliberately NOT `typography.headerFooterAlignmentFactor`** — that field is Excel-exposed, and a thin band the user cannot remove must not depend on a value someone tunes for the header. Reused rather than duplicated: `typography.fontFamily`, `headerFooterTextPadding` (right inset), `headerFooterBorderStrokeWidth` (top rule). `colophonHeight` is code-tier yet consumes layout space, so `validateLayout` must include it.
...
**Inline editing + non-destructive selection (all five inline-edit tabs).** Double-click a `data-field` cell → `.nav-cell-input`; canonical `attachInlineEditor(table, entitySingular)` driven by `INLINE_EDIT_REGISTRY` (singular → field → `'text'`/`'date'`/`'select'`, latter with an `options` getter). Date editors seed from the **canonical** stored value, not the formatted cell; commit empty → `null` (text → `''`). Escape reverts. **Selection:** a pure click routes through `selectTask`/`selectSwimlane`/`selectSimpleEntity` (in-place swap, DOM/scroll survive); a click mid-edit sets `nextSelectionIntent` and blurs to commit first — bound to `mousedown` not `click`, so selection precedes teardown. `notes` absent from the registry (multi-line needs a textarea).
...
Read-only surface for `projectData._validation` — renders on tab activation, **never re-validates**. Four panel states (undefined / clean / no-match / table); search + severity + grouping controls.
...
Left to right in an `.app-toolbar` flex row: New Project, Choose file, Save (xlsx), Save As…, Save SVG, Load Baseline, Clear Baseline, then right-aligned About.

**About** (`#aboutBtn`, always enabled) opens `<dialog id="aboutDialog">` — the **single source** of third-party license attribution (SheetJS Apache-2.0, date-fns MIT, Electron MIT — own licences only, not transitive deps; full texts in committed `LICENSES.txt`). Do not duplicate it; the desktop native menu opens the same dialog. **Hand-synced paired values (no build step, unreadable at runtime):** the About `Version` line bumps in lockstep with `package.json` `version`, and a bump also needs `npm install` to re-sync `package-lock.json`'s two root `version` fields (else they silently drift) — **only those two: a blind find-replace of the old version string across the lockfile can also hit an unrelated dependency that happens to sit on the same number** (`node_modules/pvutils` was on `1.1.5`), so match on the surrounding root keys, not the bare string; an `electron` devDependency bump must move the Electron version in BOTH the About entry and `LICENSES.txt` (same for vendored SheetJS/date-fns).

**EULA — the `License Agreement` section (About dialog).** Sits ABOVE `Third-party licenses`, reusing the same `<h3>` + `.about-license` markup, closed by a `Full text also in EULA.txt.` footer mirroring the LICENSES.txt line. **Browse-wrap only:** nothing gates app use on it — no first-launch prompt, no accept/decline flow, About opens exactly as before. **Root `EULA.txt` is the copy source and the dialog is a verbatim hand-synced duplicate** (a third paired value, and in the `files` allowlist so it ships). Sole permitted transform: one `.about-license` paragraph per clause with the clause title in the existing `.about-lib` span, and blank lines in the txt becoming paragraph breaks — never reword, summarise, or truncate, and change both sites together. The full text overflows the viewport; Chromium's UA `dialog:modal` rules (`max-height` + `overflow:auto`) already scroll it, so no sizing CSS was added.

**Scroll anchor — `<h2 tabindex="-1" autofocus>` is load-bearing, not decoration.** `showModal()`'s focusing steps otherwise land on the first focusable descendant (the SheetJS link) and the browser scrolls it into view; once the EULA grew, that opened the dialog *past* the `Version` line and the top of the agreement. Naming the heading as the autofocus target pins it at `scrollTop 0` on every open and reopen, and covers both entry paths (toolbar button, Help → About) because both just call `showModal()`. The paired `#aboutDialog h2:focus{outline:none}` rule exists ONLY because of this — the heading is a scroll anchor, not a control. Anything that lengthens the dialog above the first link re-arms the original bug, so keep the attributes and the rule together.
...
**Window/menu.** Single `BrowserWindow`, `contextIsolation: true`, `nodeIntegration: false`, `webSecurity` on. Menu keeps default roles — including **View**, a deliberate keep for the developer-and-sole-user build — plus **Help → Reload App** then **Help → About**. About reuses the EXISTING `#aboutDialog` via `showModal()` through `executeJavaScript`, guarded against not-yet-loaded (no-op) and already-open (`showModal` throws). Reload App is discoverability only — `reloadApp()` makes the same `webContents.reload()` call the View role makes, so it inherits the reload guard below with no wiring of its own. It carries **no accelerator** (`Ctrl+R` is View → Reload's) and is deliberately a `click` handler rather than `{ role: 'reload', label: … }`, since the role binds that accelerator implicitly with no clean way to strip it.

**Reload guard (`attachReloadGuard`) — main-process half of the renderer's dirty guard.** Electron drops a cancelled `beforeunload` silently instead of prompting, so without this View → Reload discards unsaved work; the listener supplies the missing confirm. Cross-tier coupling: it is load-bearing on `ui.js`'s `isDirty` `beforeunload` handler firing at all, and fires for BOTH Reload and Force Reload since `will-prevent-unload` is webContents-level and names no command. Cancel/dismiss touches the event, so the reload stays blocked. **Window-close is deliberately NOT guarded** (still Electron-default). **GOTCHA — the one-shot `allowNextUnload` flag:** Electron emits `will-prevent-unload` synchronously and reads `defaultPrevented` the instant the handler returns, so `preventDefault()` from an async `.then()` is a no-op on that event. The dialog must stay async (`showMessageBoxSync` stalls the main-process event loop), so `Reload Anyway` arms the flag and re-issues `reload()`; the re-fired event preventDefaults synchronously. A `once('did-finish-load')` disarms it — strictly after the unload decision, so it can't race the handler.
...
**Display name vs technical identifiers.** User-facing text is **`Compact Gantt`** (spaced) in four spots: `build.productName`, `index.html` `<title>`, the About `<h2>`, and the Help→About menu label. The renderer's colophon carries the domain **`compactgantt.com`** alone — no product name — and it is the only occurrence of that domain; it ships inside every exported chart and cannot be switched off, so the URL is the whole credit and must resolve to something. Technical identifiers stay spaceless/lowercase — do NOT space them: npm `name` (`compactgantt`), `build.appId` (`com.compactgantt.app`), repo name (`compactgantt_web`).
