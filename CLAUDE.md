...
**Nav-table numeric-column alignment (cross-cutting CSS).** Numeric columns are right-aligned via dedicated per-column classes on header + data cells: `task-days-col`/`task-row-col` (Tasks), `swimlane-order-col`/`swimlane-rowcount-col` (Swimlanes), `pipe-labelposition-col` (Pipes), `curtain-opacity-col` (Curtains). A class on a numeric header requires that column to break out of the otherwise-generic header loop. The `id` column is right-aligned instead by the shared `th:first-child` / `td:first-child:not(.entity-empty)` rule. **`width: 1px` shrink-to-content idiom:** caps a column to its content width so slack flows to the flexible `name`/`text` column — used on `swimlane-order-col` (+`white-space: normal` to wrap the long header), `pipe-labelposition-col` (floor = single-word header width), and the shared first-child id rule (visible only on the two-column Notes table; negligible where id already sits near content width). Links/Notes nav tables have no numeric column beyond the already-aligned `id`.
