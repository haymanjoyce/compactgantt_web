...
**Link classification (R1/R2/R3).** R1 (`pred.finishDate >= succ.finishDate`) and R2 (late-recoverable but same absolute row) mirror the renderer's invalid-link checks — neither module imports from the other. R3 (`pred.finishDate === succ.startDate` — forward link with collapsing geometry) is validation-only; the renderer's analogous skip is the geometric `origX >= termX`. Self-links (`fromTaskId === toTaskId`) skip the R1/R2/R3 block entirely; the dedicated Self-link error is the sole issue emitted for them.
