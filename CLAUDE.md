...
**`colophonInheritFooterColors` (Titles sheet, `Colophon Inherit Footer Colors` row, last row of the sheet).** Boolean, default `true`, `Yes`/`No` round-trip: `kvBool(titlesKV, 'Colophon Inherit Footer Colors', true, undefined, cfg('colophonInheritFooterColors'))`. The **only** exposed colophon field — it picks colors, never presence (see Renderer). Absent row → `true`, gained on next save. Unlike `chartDateFormat`/`tableDateFormat` it **is** in `validateTitles`'s `OWNED` set — mandatory, not optional: `kvBool` *can* emit `unrecognised_boolean`, and since each config validator filters `_parseNotices` by its own `OWNED` set with no catch-all for unconsumed notices, omitting it would make a real notice invisible in the Issues tab. (The two format fields are excluded precisely because `kvStr` can never emit one.)
...
**Structure.** A thin coordinator calls 14 per-block validators (7 entity + 7 config) in parse traversal order; the 7th entity validator is `validateBaseline` (after `validateNotes`). `validateTimeline` owns the empty-`chartDateFormat` AND empty-`tableDateFormat` errors (no `validatePreferences` — removed). `validateTitles` owns the `colophonInheritFooterColors` `unrecognised_boolean` warning (same shape as `validateLayout`'s `showRowDividers`); it has no other colophon rule, since the value is a parsed boolean with no range.

**Layout-collapse check ↔ renderer band stack (two-site sync).** `validateLayout`'s collapse error sums `paddingTop + headerHeight + scales + footerHeight + rendering.colophonHeight + paddingBottom` against `outerHeight` — it must mirror the renderer's vertical band stack term for term, including the code-tier `colophonHeight`. Drop a term and a genuinely collapsed layout goes unflagged. The scale-band sub-formula is likewise a mirror of the renderer's `bandH` computation.
...
- **Render order (painter's algorithm):** ~21 layer accumulators assembled back-to-front into 20 `<g>` groups, `colophon` last. The source's z-order comment numbers them 1–16 but reuses/omits some labels — so "slot N" references follow that comment numbering, not a strict count.
- **Vertical band stack** (bottom-up: `paddingBottom` → colophon → footer → task rows; top-down: `paddingTop` → header → scale bands → task rows). Derived as ONE chain — `colophonY = outerHeight - paddingBottom - colophonH`, `footerY = colophonY - footerHeight`, `taskRowY2 = footerY` — because `footerY` is simultaneously the footer band's top edge and the task-row area's bottom edge. The footer band **consumes `footerY`**; it must never recompute its own `y` (the two expressions were duplicates before the colophon existed, and re-deriving one of them silently overlaps the band). `taskRowY2` is the single source for ~9 downstream sites (gridlines, pipes, curtain rects/edges, badge areas, `taskRowH` → `rowH` → notes and baseline overlay), so the colophon's height propagates everywhere from that one line. `validateLayout` mirrors this sum — see Validation.
...
- **Colophon (slot 16, group `colophon`, last):** structural brand band along the bottom of the perimeter, spanning `paddingLeft → outerWidth - paddingRight` like header/footer, directly below the footer (or below the task rows when `footerHeight` is 0). **Unconditional and has no toggle anywhere** — it reserves its own space in the band stack, so nothing can overlap or obscure it and no config can remove it; the only escape is cropping the image. Right-aligned fixed literal `Compact Gantt &#183; compactgantt.com` (name + URL only — no version, no copyright mark), inset from `innerX2` by `rendering.headerFooterTextPadding`, baseline at `colophonY + colophonH × rendering.colophonAlignmentFactor`. Not project data, not an Excel field, not user-editable. Emitted **RAW, never through `escapeXml`** so the `&#183;` numeric reference survives, keeping the exported `.svg` pure ASCII regardless of what encoding a downstream consumer assumes. Top-edge `<line>` border matching the header/footer band borders (`rendering.headerFooterBorderStrokeWidth`) — with inherit ON and a footer present this reads as footer fill │ hairline │ colophon fill, deliberately mirroring the header's bottom rule. **`titles.colophonInheritFooterColors`** (default `true`) picks background/border/text from `style.headerFooter*` so the band continues the footer; `false` falls back to the three code-tier `rendering.colophon*Color` values. Font from `typography.fontFamily`. The two early-return guards emit no colophon — the one exception to "always present", and deliberate: no renderable date range means no chart to brand.
...
The colophon's six constants live here — `colophonHeight` (`14`), `colophonFontSize` (`7`), `colophonAlignmentFactor` (`0.7`), and the non-inherited fallback triple `colophonBackgroundColor` (`'#f5f5f5'`), `colophonBorderColor` (`'#d0d0d0'`), `colophonTextColor` (`'#6b6b6b'`, ~4.9:1 on the fallback background — grey-on-white at this size is illegible, so don't lighten it). `14`/`7` holds header/footer's exact 20px-band-to-10px-font ratio, so the band reads as a smaller sibling rather than a cramped afterthought. **`colophonAlignmentFactor` is deliberately NOT `typography.headerFooterAlignmentFactor`:** that field is Excel-exposed, and a band this thin that the user cannot remove must not have its legibility depend on a value someone tunes for the header. Reused rather than duplicated: `typography.fontFamily`, `rendering.headerFooterTextPadding` (right inset), `rendering.headerFooterBorderStrokeWidth` (top rule). Note `colophonHeight` is code-tier yet consumes real layout space, so `validateLayout` must include it — see Validation.
...
**Titles sub-tab sections.** `Header` → `Footer` → `Colophon`, the last a single `addCheckboxRow` for `colophonInheritFooterColors` (auto-commits on `change`, no blur cycle — same as Layout's `showRowDividers`). The colophon's presence, height, font, and fallback colors are all code-tier, so this checkbox is the only colophon control the UI exposes.
