...
**Tasks panel.** Toolbar (Add / Delete / Duplicate / Move Up / Move Down) + nav table (left) + edit form (right). Nav-table columns: `id`, `symbol`, `name`, `Days`, `startDate`, `finishDate` (no `swimlaneId` / `row` columns — both still live on the edit form). `buildTasksDisplayOrder()` sorts `(swimlane.order, task.row, finishDate, startDate, array index)`, all ascending; orphans, null `task.row`, and null/empty dates sort last within their group via Infinity / `'￿'` sentinels. Sort is display-only — `projectData.tasks` array order is never mutated, so writer round-trip preserves the user's Excel ordering. **Swimlane grouping:** the cascade-sorted tasks are bucketed by `swimlaneId` and emitted under group-header rows. Defined swimlanes render in `swimlane.order` (array) sequence, each header shown **even when its bucket is empty**; synthetic `Unassigned` (null/empty `swimlaneId`) and `Misassigned` (non-empty id, no matching swimlane) headers render only when populated. Global-empty (zero tasks) shows the standard `entity-empty` message instead of bare headers. Group headers are `<tr class="entity-nav-table-group-header">` with a single `<td colspan>` and no `data-id`, so the selection handler skips them. **Symbol cell:** CSS-drawn marker (1px black border, `box-sizing: border-box` so the border never grows the box) — `.task-symbol-milestone` (rotated 9px square ≈ 12px bar height, fixed size) vs `.task-symbol-bar` (12px tall, width set inline). Fill = `task.fillColor`, `fillPattern` ignored (solid only). Cell background = the matched swimlane's `backgroundColor` (no background for Unassigned/Misassigned rows). **Bar width is duration-proportional:** `computeBarWidth(t, maxDays)` scales `BAR_MIN_WIDTH_PX (10)`..`BAR_MAX_WIDTH_PX (54)` linearly against `maxDays` — the largest positive `taskDays` span among non-milestone tasks in the table (pre-pass). A unified `minWidth` guard covers every degenerate case (null/non-finite span, span ≤ 0, or `maxDays ≤ 0`). `BAR_MAX_WIDTH_PX` = `SYMBOL_COL_WIDTH_PX (64)` − 2×end-padding; the `64` must stay in sync with the `.task-symbol-col` CSS width in `index.html`. `taskDays(t)` is the shared numeric span helper (`formatTaskDaysCell` and the bar both consume it). **Days cell:** `daysBetween(startDate, finishDate)` behind a `Number.isFinite` guard — empty on missing/empty/non-finite dates, negatives rendered literally, milestones yield `0`; `<th title="Calendar days">`. Move Up / Move Down dispatch `update` on `task.row` (NOT the dispatcher's `moveUp`/`moveDown` array-reorder actions). Enablement uses `typeof task.row === 'number'` guards combined with `row > 1` / `row < swimlane.rowCount`, plus a "Task has no swimlane" tooltip for orphans. The row form input commits empty as `null`; non-finite input is silently dropped. Form rebuild is gated on `formRenderedForId` so commit-on-blur preserves focus; the row input is targeted-DOM-updated via `syncTaskRowInput` after Move Up / Move Down because the form does not rebuild on same-id updates.
