...
**Config panel internals.** Form-only top-level tab — no nav table, no toolbar, no selection. Seven sub-tabs in §7 order: Layout / Bars / Timeline / Titles / Style / Typography / Preferences. `renderConfigPanel(panel)` is the entry point; mirrors `renderDataPanel`'s persistent-skeleton pattern (sub-tab strip + `#configArea` survive across same-block re-renders). `activeConfigBlock` (module scope) tracks the active sub-tab; `configBlockRenderedFor` gates form rebuilds the same way `formRenderedForId` does for entity forms — form rebuilt on sub-tab switch only, so commit-on-blur dispatches preserve focus. Both vars reset alongside data-panel state in `resetDataPanelState` (function name is a soft misnomer kept for low-churn consistency). `config.rendering` is deliberately excluded from the UI; the Inspector still surfaces it via the dynamic walk. Timeline date fields commit two dispatches: the date value AND the paired `*Explicit` flag (`true` on non-empty, `false` on empty) so the writer round-trip-correctly emits user-set vs auto-derive dates. The explicit flags themselves are not user-editable and not rendered as form rows. The file-load handler symmetrically re-renders Config if active (parallel to the existing Issues re-render guard); New Project does not need this because it activates the Data tab before the seed dispatches fire.

**Form helper conventions.** `addNumberRow(form, field, value, commitFn, opts)` accepts an optional `opts = { step, min, max }`; defaults to integer-stepping (`step=1`, no bounds) so all existing call sites work unchanged. Parsing is the caller's responsibility (`parseInt` vs `parseFloat` inside `commitFn`); the helper only configures HTML attributes. Float call sites (Pipes `labelPosition`, Curtains `opacity`/`labelPosition`, Notes `xPct`/`yPct`/`widthPct`/`heightPct`) use `parseFloat` in the commit and pass `step: '0.1'` (or `'1'` for Notes) plus bounds where applicable. Enum `<select>` options use the canonical lowercase values that the parser stores (`auto/hv/vh`, `solid/dashed/dotted`, etc.) — not the §7 Excel-facing display casing. `addCheckboxRow(form, field, value, commitFn)` is a `<input type="checkbox">` wrapped in a `.checkbox-wrapper` div for left-alignment within the control column. Auto-commits on the native `'change'` event (not blur — toggling is atomic, no Escape-to-revert semantics). **Commit-argument asymmetry:** unlike the other helpers which pass the raw input string to `commitFn`, this helper passes the parsed boolean directly. Used exclusively by the Config tab: Layout `showRowDividers` and Timeline `show*` / `gridline*`. Config-tab number commits use the local `commitInt` / `commitFloat` factory helpers — empty → null, non-finite → no-op — to keep the seven form renderers terse.
